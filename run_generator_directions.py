...
def generate_images(network_pkl, seeds, truncation_psi):
    ...
    i = 0
    j = 0
    direction = np.load("stylegan2_directions/smile.npy")
    while True:
        w = w - direction * i
        images = Gs.components.synthesis.run(w, **Gs_kwargs) # [minibatch, height, width, channel]
        # images = Gs.run(z, None, **Gs_kwargs) # [minibatch, height, width, channel]
        PIL.Image.fromarray(images[0], 'RGB').save(dnnlib.make_run_dir_path('seed%04d.png' % j))
        j += 1
        i += 0.01
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    func_name_map = {
        'generate-images': 'run_generator_directions.generate_images',
        'style-mixing-example': 'run_generator_directions.style_mixing_example'
    }
