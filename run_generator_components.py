...
def generate_images(network_pkl, seeds, truncation_psi):
    ...
    for i in range(10):
        ...
        rnd = np.random.RandomState(seeds[0]+i)
        z = rnd.randn(1, *Gs.input_shape[1:]) # [minibatch, component]
        tflib.set_vars({var: rnd.randn(*var.shape.as_list()) for var in noise_vars}) # [height, width]
        w = Gs.components.mapping.run(z, None)
        direction = np.load("stylegan2_directions/age.npy")
        images = Gs.components.synthesis.run(w, **Gs_kwargs) # [minibatch, height, width, channel]
        # images = Gs.run(z, None, **Gs_kwargs) # [minibatch, height, width, channel]
        PIL.Image.fromarray(images[0], 'RGB').save(dnnlib.make_run_dir_path(f'seed{seeds[0]+i}.png'))
        components = []
        for idx in range(direction.shape[0]):
            unit_dir = np.divide(direction[idx], np.sqrt(np.dot(direction[idx], direction[idx])))
            components.append(np.dot(w[0][idx], unit_dir))
        print('component along each layer : ', components)
        print('average component : ', sum(components) / len(components))
        print('----------------------------------------------------------------')
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    func_name_map = {
        'generate-images': 'run_generator_components.generate_images',
        'style-mixing-example': 'run_generator_components.style_mixing_example'
    }
